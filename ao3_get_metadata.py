...
multichap_only = ""
tags = []
metadata = [["work_id", "title", "author", "category", "fandom", "relationship", "character", "additional_tags", "language", "date updated", "status", "words", "chapters", "comments", "kudos", "bookmarks", "hits"]]
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description='Scrape AO3 work IDs given a search URL')
    parser.add_argument(
        'url', metavar='URL',
...
def get_ids(header_info=''):
    ...
    for tag in works:
        if (multichap_only):
            ...
            t = t[5:]
            if not t in seen_ids:
                fandom = tag.find('h5', class_="fandoms").find('a').text
                title_author = [x.text for x in tag.select('.header > .heading > a')]
                title = title_author[0]
                author = title_author[1]

                datetime = tag.find("p", class_="datetime").text
                required_tags = [x.get("title") for x in tag.find("ul", class_="required-tags").find_all("span")]
                rating = required_tags[0]
                category = required_tags[4]
                status = required_tags[6]

                warning_tags = tag_list_to_string([x.find("a").text for x in tag.find('ul', class_='tags').find_all("li", class_="warnings")])
                relationship_tags = tag_list_to_string([x.find("a").text for x in tag.find('ul', class_='tags').find_all("li", class_="relationships")])
                character_tags = tag_list_to_string([x.find("a").text for x in tag.find(
                    'ul', class_='tags').find_all("li", class_="characters")])
                additional_tags = tag_list_to_string([x.find("a").text for x in tag.find(
                    'ul', class_='tags').find_all("li", class_="freeforms")])

                language = tag.find("dd", class_="language").text
                words = tag.find("dd", class_="words").text
                chapters=tag.find("dd", class_="chapters").text
                comments = tag.find("dd", class_="comments").text if tag.find("dd", class_="comments") else 0
                kudos = tag.find("dd", class_="kudos").text if tag.find("dd", class_="kudos") else 0
                bookmarks = tag.find("dd", class_="bookmarks").text if tag.find("dd", class_="bookmarks") else 0
                hits = tag.find("dd", class_="hits").text if tag.find("dd", class_="hits") else 0

                row = [t, title, author, rating, category, fandom, relationship_tags, character_tags, additional_tags, warning_tags, language, datetime, status, words, chapters, comments, kudos, bookmarks, hits]
                metadata.append(row)
                ids.append(t)
                seen_ids.append(t)
    print(f"Pulled {len(works)} fics.")
    return ids

def tag_list_to_string(lot):
    return " , ".join(lot)
...
# 
# Write all of the metadata to a csv
# 
def write_metadata_to_csv():
    global num_recorded_fic, metadata
    with open(csv_name + "_metadata.csv", 'a') as csvfile:
